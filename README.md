# 🌐 Webアプリケーション開発ポートフォリオ

## 📘 概要
- 利用者が業種を選び、シナリオに対して選択肢を選ぶ形式のトレーニングアプリです。
- 選択肢ごとにフィードバックが表示され、適切な対応を学ぶことができます。
- 支援施設や職業訓練の現場での活用を想定し、シンプルでカスタマイズしやすい構成になっています。



---

## 🛠 開発環境
- HTML / CSS / JavaScript
- JSON（シナリオ管理用）
- 推奨ブラウザ：Google Chrome / Microsoft Edge
- 開発・動作確認：Visual Studio Code + Live Server



---

## 🧩 機能一覧
- 業種選択（飲食店、スーパー、洋服屋、コンビニ、書店、クリーニング店、清掃業、軽作業など）
- シナリオ表示と選択肢による回答
- 選択肢に応じたフィードバック表示
- 「次のシナリオへ」ボタンで複数の場面を連続練習
- シナリオデータは `scenarios.json` で管理し、自由に追加・編集可能


---

## 📂 ディレクトリ構成
```
customer-sim
├── index.html
├── style.css
├── script.js
├── scenarios.jason
├── guide.html
└── README.md


---


## 🧠 設計方針・工夫点
- **業種ごとの柔軟な拡張性**：HTMLファイルに新しい業種、JSONファイルにシナリオを追加するだけで 対応可能
- **視覚的にわかりやすいUI**：利用者が迷わず操作できるよう、画面遷移を最小限に
- **フィードバックの質にこだわり**：◎○△×などの記号で直感的に理解できるよう工夫
- **職員による編集も想定**：非エンジニアでも `scenarios.json` を編集しやすい構造に
                          職員向けに設定ガイドのページ'guide.html'を作成


---


## 🧭 今後の拡張予定
- 難易度やタグによるシナリオ分類（例：level: easy / tag: マナー）
- 音声読み上げ機能（Web Speech API）
- スコア表示や履歴記録機能
- スマートフォン対応のUI最適化
- CSVやスプレッドシートからのシナリオ一括読み込み


---


## 🧾 ライセンス・著作権
このプロジェクトは学習目的で作成したものであり、商用利用は想定していません。  
各種ライブラリ・ツールのライセンスはそれぞれの作者に帰属します。

---

## 👤 作成者
- **氏名（またはGitHubアカウント）**：Sachi T
- **開発期間**：2025年10月〜11月   
- **GitHub**：https://github.com/sachi-t

---

## ✅ 最終更新日
2025-11-20
